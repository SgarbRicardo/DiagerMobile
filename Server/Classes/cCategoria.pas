...
begin
  try
      ...
      qry.Connection := FConn;
        ...
        begin
          Active := false;
          sql.Add('SELECT distinct C.Cat_Id, C.CAT_DESCRIPTION, count (c.cat_id) as qtd_Cate');
          sql.Add('FROM [MF_Control].[mf_control_db].[CATEGORY] C');
          sql.Add('JOIN [MF_Control].[mf_control_db].[EQUIPMENT] E ON (E.Cat_Id = C.Cat_Id)');
          sql.Add('JOIN [MF_Control].[mf_control_db].[LEASING] L ON (L.Empre_Equip_Id = E.EQUIP_ID)');
          sql.Add('JOIN [MF_Control].[mf_control_db].userP U ON (u.user_id = L.Empre_User_Id)');
          sql.Add('where 1=1');
          ...
          begin
           ...
           sql.Add('group by C.Cat_Id, C.CAT_DESCRIPTION');
