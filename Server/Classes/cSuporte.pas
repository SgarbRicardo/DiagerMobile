      ...
      private
        ...
        FCALL_ID: INTEGER;
         public
           ...
           function ListaVersaoAtend(order_by: string; out erro: string): TFDQuery;
...
function TSuporte.ListaVersaoAtend(order_by: string; out erro : string): TFDQuery;
var
  qry : TFDQuery;
begin
  try
      ...
      qry.Connection := FConn;
        ...
        begin
          Active := false;
          sql.Add('SELECT version, count (version) as Versao');
          sql.Add('FROM [MF_Control].[mf_control_db].[support]');
          sql.Add('Where version <> ''''');
          sql.Add('group by version');

        if order_by = '' then
            SQL.Add('ORDER BY version ASC')
        else
    ...
    begin
        erro:= 'Erro ao carregar a lista de versao de banco mais atendida : ' + ex.Message;
        result := nil;
    end;
...
begin
  try
      ...
      with qry do
        begin
          Active := false;
          sql.Add('SELECT distinct status, count (*) as quantidade');
          sql.Add('FROM [MF_Control].[mf_control_db].[support]');
...
begin
  try
      ...
      with  qry do
        begin
          Active := false;
          sql.Add('select TOP (10) call_id,telephonist,convert (char,call_date,14) as hora, scanner, version, reason, phone,occurrence ,solution');
          sql.Add('FROM [MF_Control].[mf_control_db].[support]');
  ...
  try
      ...
      with  qry do
        begin
          ...
          sql.Add('FROM [MF_Control].[mf_control_db].[support]');
          sql.Add('WHERE status like ''%Wait for CR%''');
