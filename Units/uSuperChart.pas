...
type
  ...
  public
    ...
    procedure LoadAtendVerFromJSON(jsonString: string; var error: string);
...
begin
    ...
    for x := jsonArray.Size - 1 downto 0 do
        if MaxValue < jsonArray.Get(x).GetValue<double>('quantidade') then
            MaxValue := jsonArray.Get(x).GetValue<double>('quantidade');
    ...
    begin
        for x := jsonArray.Size - 1 downto 0 do
        begin
            AddBar(jsonArray.Get(x).GetValue<double>('quantidade'),
    ...
    begin
        ...
        for x := 0 to jsonArray.Size - 2 do
            AddLine(jsonArray.Get(x).GetValue<double>('quantidade'),
                    jsonArray.Get(x+1).GetValue<double>('quantidade'));


        CountComp := 0;
        for x := 0 to jsonArray.Size - 1 do
            AddLinePoint(jsonArray.Get(x).GetValue<double>('quantidade'),
                         jsonArray.Get(x).GetValue<string>('status'));
...
procedure TSuperChart.LoadAtendVerFromJSON(jsonString: string; var error: string);
var
    jsonArray: TJsonArray;
    x : integer;
    p: TPoints;
begin
    ...
    for x := jsonArray.Size - 1 downto 0 do
        if MaxValue < jsonArray.Get(x).GetValue<double>('Versao') then
            MaxValue := jsonArray.Get(x).GetValue<double>('Versao');
    ...
    begin
        for x := jsonArray.Size - 1 downto 0 do
        begin
            AddBar(jsonArray.Get(x).GetValue<double>('Versao'),
                   jsonArray.Get(x).GetValue<string>('version'));
        end;
    end
    else if (FChartType = Lines) then
    begin
        ...
        for x := 0 to jsonArray.Size - 2 do
            AddLine(jsonArray.Get(x).GetValue<double>('Versao'),
                    jsonArray.Get(x+1).GetValue<double>('Versao'));


        CountComp := 0;
        for x := 0 to jsonArray.Size - 1 do
            AddLinePoint(jsonArray.Get(x).GetValue<double>('Versao'),
                         jsonArray.Get(x).GetValue<string>('version'));
