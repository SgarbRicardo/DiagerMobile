...
begin
    ...
    try
      ...
      usuario := jsonObj.GetValue('user').Value;
...
begin
  ...
  try
    ...
    begin
        ...
        pwd4 := password from [MF_Control].[mf_control_db].[userP]
        where [user] ='" + dados.User + "'

        select pwdCompare(@pwd3, @pwd4, 0)
      //RequestLogin.AddParameter('senha',edt_pwd.Text);
     // RequestLogin.AddParameter('user',edt_user.Text);
     // RequestLogin.AddParameter('senha', MD5(edt_pwd.Text));
      RequestLogin.ExecuteAsync(ProcessaLogin, true, true, ProcessaLoginErro);
    end
    else
    begin
    // consumir WS de criação de conta...
        foto64 := Base64FromBitmap(cfoto.Fill.Bitmap.Bitmap);

      RequestConta.Params.Clear;
      RequestConta.AddParameter('email', edtEmail.Text);
      RequestConta.AddParameter('user', edt_cadUser.Text);
      RequestConta.AddParameter('senha',edtCadPwd.Text);
     // RequestConta.AddParameter('senha', MD5(edtCadPwd.Text));
      RequestConta.AddParameter('foto', foto64, pkREQUESTBODY);
